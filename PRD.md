...
- **Development vs. runtime distinction:** Claude Code (Anthropic's agentic coding CLI, built on the Claude Agent SDK) was used as the development agent throughout — it designed and implemented the retrieval pipeline, LLM provider abstraction, orchestration/routing logic, essay/artifact generation, and frontend (session transcripts in `agent-transcripts/`). That is a real, substantial use of Claude's agentic tooling, but it operated at build time, not runtime: the deployed FastAPI application itself does not call Claude Code or the Agent SDK when handling a user's request.
- At runtime, this implementation instead uses a small custom `Orchestrator` (`src/services/orchestrator.py`) that routes by keyword detection, with each FastAPI endpoint (`/chat`, `/essays`, `/artifacts`) calling the retrieval and LLM services directly — the same skill boundaries and routing shape a runtime agent-SDK integration would have, just framework-free
